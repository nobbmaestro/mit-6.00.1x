...
def calcAnnualBalance(balance, annualInterestRate, monthlyPaymentRate):
    """Return the credit card balance after one year.

    Assumes that person only pays the minimum monthly payment required by the
    credit card company each month.

    Args:
        balance (float): the outstanding balance on the credit card
        annualIntesestRate (float): annual interest rate as a decimal
        monthlyPaymentRate (float): minimum monthly payment rate as a decimal

    Returns:
        float: credit card balance.
    """
    for _ in range(12):
