...
the monthly payment is a multiple of $0.01). Does your code still work? It should, but you may notice that your
code runs more slowly, especially in cases with very large balances and interest rates. (Note: when your code is
running on our servers, there are limits on the amount of computing time each submission is allowed, so your
observations from running this experiment on the grading system might be limited to an error message complaining
...
into the problem of slow code? We can make this program run faster using a technique introduced in lecture - bisection
search!
...
def getBalance(balance, annualInterestRate, minimumFixedMonthlyPayment):
    """Auxiliary  function for min_montly_payment_bisect()."""
    for _ in range(12):
        monthlyIntresentRate = annualInterestRate / 12.0
...
def minMonthlyPaymentBisect(balance, annualInterestRate):
    """Return the minimum fixed monthly payment needed by bisection search.
    ...
    Returns:
        float: minimun fixed mountly payment
    """
    epsilon = 0.001
    low = balance / 12
    high = (balance * (1 + annualInterestRate/12.0)**12) / 12.0
    minimumFixedMonthlyPayment = (low+high) / 2.0

    while abs(getBalance(balance, annualInterestRate, minimumFixedMonthlyPayment)) >= epsilon:
        if getBalance(balance, annualInterestRate, minimumFixedMonthlyPayment) > 0:
            low = minimumFixedMonthlyPayment
        else:
            high = minimumFixedMonthlyPayment
        minimumFixedMonthlyPayment = (low+high) / 2.0

    return round(minimumFixedMonthlyPayment, 2)
