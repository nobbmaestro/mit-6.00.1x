...
contains a helper function get_story_string() that returns the encrypted version of the story as a string. Create a
CiphertextMessage object using the story string and use decrypt_message to return the appropriate shift value and
...
from ProblemSet5.lib import get_story_string
...
def decrypt_story(story_text=get_story_string()):
    ...
    Args:
        story_text (str, optional): _description_. Defaults to get_story_string().

    Returns:
        CephertextMessage: _description_
    """
    return CiphertextMessage(story_text).decrypt_message()


def main():
    """Initialize decrypt story."""
